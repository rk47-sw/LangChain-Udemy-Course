...
Usage:
    python convert_to_gemini.py --all              # Convert all files
    python convert_to_gemini.py --file path.py     # Convert single file
    python convert_to_gemini.py --notebooks         # Convert only notebooks
    python convert_to_gemini.py --preview           # Preview changes without applying
"""
...
class GeminiConverter:
        ...
        try:
            ...
            if content != original:
                if not preview_only:
                    ...
                    print(f"   Lines changed: {len(original.splitlines()) - len(content.splitlines())}")
        ...
        try:
            ...
            if json.dumps(notebook) != original:
                if not preview_only:
                    ...
                    print(f"   Code cells with changes: {code_cells_changed}")
                
                self.changes.append(str(notebook_path))
                return True
            else:
                print(f"ℹ️  No changes needed: {notebook_path}")
                return False
    ...
    def convert_all(self, preview_only=False):
        ...
        print(f"\n📝 Found {len(py_files)} Python files and {len(nb_files)} notebooks\n")
